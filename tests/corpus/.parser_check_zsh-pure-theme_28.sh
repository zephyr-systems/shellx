...
:
				...
				if [ "$(__shellx_list_get info top)" = "$(__shellx_list_get prompt_pure_vcs_info top)" ]; then
				  case "$(__shellx_list_get prompt_pure_vcs_info pwd)" in
				    "$PWD"*) __shellx_list_set_index prompt_pure_vcs_info pwd "$PWD" ;;
				  esac
				else
