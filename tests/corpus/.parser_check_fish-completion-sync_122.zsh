...
fish_completion_sync() {
	FISH_COMPLETION_DATA_DIRS=()
	for addition in ""; do
		if set fish_complete_path $(fish_completion_sync_filter $addition $fish_complete_path); then
			FISH_COMPLETION_ADDITIONS=()
		fi
	done
	for data_dir in ""; do
		if set -a fish_complete_path $data_dir; then
			FISH_COMPLETION_ADDITIONS=$data_dir
		fi
:
}
