...
function _zsh_nvm_completion() {
	  # Add provided nvm completion
	  [[ -r $NVM_DIR/bash_completion ]] && source $NVM_DIR/bash_completion
}
function _zsh_nvm_lazy_load() {
	  ...
	  if [[ "$NVM_NO_USE" == true ]]; then
	    ...
	    __shellx_list_set global_binaries $(_zsh_nvm_global_binaries)
	  fi
	  # Add yarn lazy loader if it's been installed by something other than npm
	  _zsh_nvm_has yarn && __shellx_list_append global_binaries 'yarn'
	  # Add nvm
	  __shellx_list_append global_binaries 'nvm'
	  __shellx_list_append global_binaries $NVM_LAZY_LOAD_EXTRA_COMMANDS
	  # Remove any binaries that conflict with current aliases
	  local cmds
	  cmds=""
	  local bin
	  for bin in $global_binaries; do
	    [[ "$(which $bin 2> /dev/null)" = "$bin: aliased to "* ]] || __shellx_list_append cmds $bin
	  done
...
function _zsh_nvm_upgrade() {
	  ...
	  latest_version=$(_zsh_nvm_latest_release_tag)
...
function _zsh_nvm_previous_version() {
	  cat "$ZSH_NVM_DIR/previous_version" 2>/dev/null
}
function _zsh_nvm_revert() {
	  previous_version="$(_zsh_nvm_previous_version)"
...
function _zsh_nvm_install_wrapper() {
	  case $2 in
	    'rc')
